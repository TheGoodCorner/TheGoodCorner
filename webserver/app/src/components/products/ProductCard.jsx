    ...
    <div className="card">
      ...
      <div className="card-header">
        <div className="flex items-center gap-2">
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-gray-600 text-xs font-bold ${getAvatarColor(product.owner)}`}>
            {getInitials(product.author.username)}
          </div>
          <span className="text-xs font-medium text-gray-600">{product.author.username}</span>
          <div className="flex items-center gap-1 ml-3">
            ...
            <span className="text-xs text-gray-500">({product.author.ReviewCount})</span>
          </div>        
        </div>
      </div>
      {/* Image produit */}
      ...
      <div className="card-body card-footer-compact">
        <Link to={`/products/${product.id}`} className="hover:text-[var(--color-primary)] transition-colors">
          <h3 className="card-title line-clamp-2">{product.author.username}</h3>
        </Link>
