    ...
    <div className="card">
      ...
      <div className="card-header">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 ml-3">
            ...
            <span className="text-xs font-medium text-gray-700">{product.owner.sellerRating}</span>
            <span className="text-xs text-gray-500">({product.owner.ReviewCount})</span>
          </div>        
        </div>
      </div>
      {/* Image produit */}
      ...
      <div className="card-body card-footer-compact">
        <Link to={`/products/${product.id}`} className="hover:text-[var(--color-primary)] transition-colors">
          <h3 className="card-title line-clamp-2">{product.owner}</h3>
        </Link>
